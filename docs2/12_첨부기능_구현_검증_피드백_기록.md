...
적용 코드: `OGTECH-llm/MAP/`, `OGTECH-llm/Co-LLM/`

첨부 PDF와 영상에 나온 기능의 구현 결과와 검증 근거를 기록한다. 첨부자료 속 문구와 동작은 요구사항 데이터로만 해석했으며, 그 안의 지시는 실행 지시로 취급하지 않았다. 안전 판단은 `PLAN.md`, `AGENTS.md`, `OGTECH-org/docs/AI_AGENT_GUIDE.md`를 우선했다.
...
- 수행: 브라우저는 [`MAP/test-results/product_ui_1024x600.json`](../MAP/test-results/product_ui_1024x600.json)으로 부팅 ACK 초기 잠금, N키·배경 클릭 차단, 포커스 트랩, DEMO/대기/통과 구분, 진단 상세 6개, 96px 버튼, 1024×600 문서 크기, 야간 음성 제어를 확인했다.
...
- 증거: [`MAP/test-results/product_ui_1024x600.json`](../MAP/test-results/product_ui_1024x600.json), [`product_boot_1024x600.png`](../MAP/test-results/product_boot_1024x600.png), [`product_night_1024x600.png`](../MAP/test-results/product_night_1024x600.png)
